...
export const generateTemplateContent = (formData, templateType) => {
  ...
  if (templateType === 1) {
    ...
    <div style="width: 60%; padding-right: 20px;">
        ...
        <div style="display: flex; align-items: center; margin-bottom: 20px; margin-left: 15px;">
            <div style="background-color: #2c3e50; color: white; width: 70px; height: 70px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 28px;">
                ${formData.name ? formData.name[0] : 'J'}
            </div>
            <div style="margin-left: 20px;">
                <h1 style="margin: 0; font-size: 24px;">${formData.name || 'John Smith'}</h1>
        ...
        <div style="margin-top: 30px; margin-left: 55px;">
            ...
            ${formData.experience.length > 0 ? formData.experience.map(exp => `
                <div style="margin-left: 20px; margin-bottom: 15px; position: relative;">
                    ...
                    <strong>${exp.position || 'Position not provided'}</strong> at <strong>${exp.company || 'Company not provided'}</strong>
                    <p style="margin-left: 20px; font-size: 14px; color: #333; display: flex; align-items: center;">
                        <span style="background-color: #2c3e50; width: 10px; height: 10px; border-radius: 50%; display: inline-block; margin-right: 10px;"></span>
        ...
        <div style="margin-top: 30px; margin-left: 55px;">
            ...
            ${formData.education.length > 0 ? formData.education.map(edu => `
                <div style="margin-left: 20px; margin-bottom: 15px; position: relative;">
                    <p style="font-size: 12px; color: #555; position: absolute; left: -75px; top: 0;">${edu.dates || 'Date not provided'}</p>
                    <strong>${edu.degree || 'Degree not provided'}</strong> at <strong>${edu.institution || 'Institution not provided'}</strong>
                    <p style="margin-left: 20px; font-size: 14px; color: #333; display: flex; align-items: center;">
                        <span style="background-color: #2c3e50; width: 10px; height: 10px; border-radius: 50%; display: inline-block; margin-right: 10px;"></span>
                        ${edu.details || 'Details not provided'}
                    </p>
                </div>
    ...
    <div style="width: 40%; padding-left: 20px; padding: 20px;">
      ...
      <div style="font-size: 14px; color: #333;">
          ...
          <div style="margin-bottom: 10px;">
              ...
              <div style="margin-left: 20px;">${formData.linkedin || 'linkedin.com/johnutw'}</div>
          </div>
      </div>

      <!-- Languages Section -->
      <h3 style="font-size: 20px; margin-top: 30px; margin-bottom: 20px; display: flex; align-items: center;">
      ...
      <div style="font-size: 14px; color: #333;">
          ${formData.languages.length > 0 ? formData.languages.map((lang, index) => `
              <div key="${index}" style="margin-bottom: 10px;">
                  <strong style="display: block;">${lang.language || 'Language not provided'}:</strong>
                  <div style="margin-left: 20px;">${lang.level || 'Level not provided'}</div>
              </div>
